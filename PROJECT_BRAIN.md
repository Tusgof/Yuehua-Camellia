...
- Webull: เจ้าของยืนยันคำขอ OpenAPI ในแอปแล้ว; Camellia ตรวจ production read-only สำเร็จวันที่ `2026-09-25` ด้วย `py -3.11 scripts/check_webull_read_only.py` บน Python 3.11.9 / SDK 2.0.13 / region `th`: account list, balance, positions และ instrument metadata ตอบกลับสำเร็จ; VTI, EWJ, VWO, GLD, XLE, SHY, DBC, IPAC คืน `status=OC` และ `fractionable=true` ทุกตัว ไม่มี order call. ไม่บันทึกข้อมูลบัญชีหรือ credential ลง repo; ผลนี้ยังไม่ยืนยันสิทธิ์ส่งคำสั่งจริง
