...
- Webull: เจ้าของยืนยันคำขอ OpenAPI ในแอปแล้ว; Camellia ตรวจ production read-only สำเร็จวันที่ `2026-09-25` ด้วย `py -3.11 scripts/check_webull_read_only.py` บน Python 3.11.9 / SDK 2.0.13 / region `th`: account list, balance, positions และ instrument metadata ตอบกลับสำเร็จ; VTI, EWJ, VWO, GLD, XLE, SHY, DBC, IPAC คืน `status=OC` และ `fractionable=true` ทุกตัว ไม่มี order call. ต่อมา production preview สำเร็จสำหรับ VTI `0.01` หุ้นแบบ MARKET/DAY/CORE และคืน estimated cost/fee; preview ของ MOO ถูกปฏิเสธด้วย `OPENAPI_PARAM_ERR`; ทั้งหมด orders_sent=0. ไม่บันทึกข้อมูลบัญชีหรือ credential ลง repo
